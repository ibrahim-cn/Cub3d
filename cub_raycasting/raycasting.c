...
void	raycasting(t_cub3d *cub)
{
	...
	{
		// ---------------------------------------------------------
		// 1. Işın Pozisyonu, Yönü ve Harita Konumu
		// ---------------------------------------------------------
		...
		// ---------------------------------------------------------
		// 2. DDA Hazırlığı (Delta Dist ve Step)
		// ---------------------------------------------------------
		...
		// ---------------------------------------------------------
		// 3. DDA Algoritması (Duvar Bulana Kadar İlerle)
		// ---------------------------------------------------------
		int hit = 0;
		while (hit == 0)
		{
			if (cub->ray.side_dist_x < cub->ray.side_dist_y)
			{
				...
				cub->ray.side = 0; // X eksenine çarptı (Doğu/Batı)
			}
			else
			{
				cub->ray.side_dist_y += cub->ray.delta_dist_y;
				cub->ray.map_y += cub->ray.step_y;
				cub->ray.side = 1; // Y eksenine çarptı (Kuzey/Güney)
			}
			tile = get_map_cell(cub, cub->ray.map_y, cub->ray.map_x);
			if (tile == '1')
				hit = 1;
		}
		...
		// ---------------------------------------------------------
		// 5. TEXTURE SEÇİMİ VE HESAPLAMALAR (YENİ KISIM)
		// ---------------------------------------------------------
		...
		// wall_x: Işının duvarda tam olarak nereye çarptığı (0 ile 1 arası)
		if (cub->ray.side == 0)
		...
		// tex_x: Texture üzerindeki X koordinatı
		...
		// Texture adım hesabı
		step = 1.0 * TEX_HEIGHT / cub->ray.line_height;
		...
		{
			tex_y = (int)tex_pos & (TEX_HEIGHT - 1); // Texture boyutu 2'nin kuvveti olmalı (64 gibi)
			tex_pos += step;
